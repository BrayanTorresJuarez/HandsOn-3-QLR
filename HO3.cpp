...
void calcular_regresion_cuadratica(double x[], double y[], double  n, double &a, double &b, double  &c) {
    ...
    cout <<"--- Listado de las Sumactorias ---"<<endl;
    ...
    double denom = ((sum_x2*sum_x2*sum_x2)+(sum_x*sum_x*sum_x4)+(n*sum_x3*sum_x3))-((n*sum_x2*sum_x4)+(sum_x*sum_x3*sum_x2)+(sum_x2*sum_x*sum_x3));
...
int main() {
    ...
    //double x[]= {15,20,25,30,35,40,45,50,55,60,65,70,75};
    //double y[]= {22.3,25.5,27.5,29.0,28.8,30.0,29.9,30.2,30.4,28.8,27.4,25.3,23.3};
    
    double SSE = 0, SST = 0, hat_y = 0, maybeR;

    // Remember change this for each case, is the length
    int n = 13;
    double a, b, c,input,result;
